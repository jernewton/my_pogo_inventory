...
const finishTargetXP  = 0; // 30 million  15,123,414 
...
//   .attr("fill", "purple")
//   .style("font-size", "12px")
//   .text(`${(xpAtTomorrow-currPace.xp).toLocaleString()}`);
...
//   .attr("fill", "purple")
//   .style("font-size", "12px")
//   .text(`${(xpAtTomorrow2-currPace.xp).toLocaleString()}`);
  ...
  .attr("x", 20).attr("y", 50)
  .attr("alignment-baseline", "middle")
  .text("Linear projection: " + d3.timeFormat("%m/%d")(projectedDate));
...
//   .attr("x", 20).attr("y", 70)
//   .attr("alignment-baseline", "middle")
//   .text("Tomorrow's projected Amount");
...
//   .attr("x", 20).attr("y", 90)
//   .attr("alignment-baseline", "middle")
//   .text("Polynomial projection: " + d3.timeFormat("%m/%d")(projectedDate_poly));
...
const expData = data
  .filter(d => d.xp > 0) // log requires positive values
  .map(d => [
    (d.timestamp.getTime() - t0) / (1000 * 60 * 60 * 24),
    d.xp
  ]);
...
function exponentialRegression(points) {
  ...
  return (x) => A * Math.exp(-k * x);
}

// Fit exponential model
const expFunc = exponentialRegression(expData);

// Projection range
let day = expData[expData.length - 1][0];
...
// Build curve data
const expLineData = d3.range(0, day, 1).map(d => ({
  timestamp: new Date(t0 + d * 24 * 60 * 60 * 1000),
  xp: expFunc(d)
}));
...
// Optional: mark near-zero threshold
const threshold = 1;

let thresholdDay = 0;
while (expFunc(thresholdDay) > threshold && thresholdDay < 1000) {
  thresholdDay += 1;
}

const projectedDate_exp = new Date(
  t0 + thresholdDay * 24 * 60 * 60 * 1000
);

svg.append("circle")
  .attr("cx", x(projectedDate_exp))
  .attr("cy", y(threshold))
  .attr("r", 5)
  .attr("fill", "orange");

svg.append("text")
  .attr("x", x(projectedDate_exp) + 5)
  .attr("y", y(threshold) - 5)
  .text(d3.timeFormat("%b %d, %Y")(projectedDate_exp))
  .attr("fill", "orange");

// Legend
legend.append("line")
  .attr("x1", 0).attr("y1", 70)
  .attr("x2", 12).attr("y2", 70)
  .attr("stroke", "orange")
  .attr("stroke-dasharray", "6 3")
  .attr("stroke-width", 2);
